...
- 홈 게임 카드는 ≥600px에서 2열.
- 홈 맨 위 카드(연속 기록)는 **누르면 게임 하나를 골라 바로 시작**하는 버튼이다 (`engine/pickGame.ts`).
...
## 아이디어 보관함 (아직 안 함 — 하기로 정한 것 아님)

### 친구와 겨루기 (소셜/랭킹)
카톡 게임처럼 친구를 제치고 올라가는 재미. **일부러 미뤄둔 기능** — 무거워지기도 하고,
치매 예방이라는 목적과 충돌할 수 있어서(지면 위축돼 그만두게 됨) 하게 되면 아래를 지킨다.
- **지는 사람이 안 생기게**: 1~10등 줄세우기 말고 "이번 주 우리 셋 다 목표 달성 🎉" 같은 협력형,
  또는 본인의 지난주 기록과 겨루기(자기 자신이 상대)를 기본으로.
- **초대는 링크 하나로**: 친구 추가·검색 UI 금지. 카톡으로 링크 보내고 누르면 같은 방에 들어오는 정도.
- 서버 일감: `groups` / `group_members` / 주간 집계 뷰, RLS(같은 방 사람만 서로 봄), 초대 코드.
- 넣더라도 **기본은 꺼짐**. 설정에서 켠 사람만 보이게.

### 그 밖에 나온 이야기
- 오늘의 지남력 체크(날짜·요일 출석 확인) — 게임보다는 홈/콘텐츠 슬롯 성격
